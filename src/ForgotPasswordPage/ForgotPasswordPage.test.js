...
describe('ForgotPasswordPage', () => {
  ...
  it('displays the logo', () => {
    const { getByTestId } = render(<ForgotPasswordPage navigation={mockNavigation} />)
    const logo = getByTestId('forgot-password-logo')
    expect(logo).toBeTruthy()
  })

  it('displays the title', () => {
    const { getByText } = render(<ForgotPasswordPage navigation={mockNavigation} />)
  ...
  it('sends a password reset email', async () => {
    const { getByTestId } = render(<ForgotPasswordPage navigation={mockNavigation} />);
    fireEvent.changeText(getByTestId('forgotpass-email-input'), 'test@example.com');
