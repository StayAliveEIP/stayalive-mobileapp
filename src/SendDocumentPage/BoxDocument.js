...
import { ActivityIndicator, Image, Text, TouchableOpacity, View, Platform, PermissionsAndroid } from "react-native";
...
export function BoxDocument(props) {
    ...
    const handleDocumentPick = async () => {
        try {
            ...
            Snackbar.show({
                text: 'Une erreur est survenue lors de la sélection du document',
    ...
    const handleDeleteDocument = async () => {
        ...
        try {
            const apiUrl = `${urlApi}/rescuer/document?type=${props.type}`;
            ...
            props.setData(null);
            console.error('Erreur lors de la tentative de suppression d\'un document : ', error);
        } finally {
    ...
    const handleDownloadDocument = async () => {
        ...
        try {
            const apiUrl = `${urlApi}/rescuer/document/download?type=${props.type}`;
            const token = await AsyncStorage.getItem('userToken');
            ...
            RNFetchBlob.config({
                ...
                addAndroidDownloads: {
                    ...
                    mediaScannable: true,
                    title: props.type,
                    path: `${RNFetchBlob.fs.dirs.DownloadDir}/${props.type}`,
                },
            })
                .fetch('GET', apiUrl, {
                    Authorization: `Bearer ${token}`,
                    ...
                    key !== "uri" && (
                        <View key={key} style={{ flexDirection: 'column' }}>
                            <View style={{ flexDirection: 'row' }}>
                                <Text style={{ fontSize: 16, color: colors.StayAliveRed, fontWeight: 'bold', marginRight: 5 }}>
                                    {key}:
                                </Text>
                                {key === "size" ? (
                                    <Text style={{ fontSize: 16, color: 'black', marginRight: 5 }}>
                                        {value === null ? "Aucune information" : `${(value / 1024).toFixed(2)} Ko`}
                                    </Text>
                                ) : (
                                    <Text style={{ fontSize: 16, color: 'black', marginRight: 5 }}>
